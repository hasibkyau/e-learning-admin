...
export class FileFolderService {
  ...
  getAllBasicFileFolders(searchQuery?: string) {
    let params = new HttpParams();
    if (searchQuery) {
      params = params.append('q', searchQuery);
    }
    return this.httpClient.get<{ data: FileFolder[], count: number, success: boolean }>(API_BRAND + 'get-all-basic', {params});
  }
  ...
  getFileFolderById(id: string, select?: string) {
    ...
    return this.httpClient.get<{ data: FileFolder, message: string, success: boolean }>(API_BRAND + 'get-by/' + id, {params});
  }
