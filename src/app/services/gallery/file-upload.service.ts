...
const API_UPLOAD = environment.ftpBaseLink + (environment.ftpPrefix ? `/${environment.ftpPrefix}/upload/` : '/upload/');
const API_V2_UPLOAD = environment.ftpBaseLink + (environment.ftpPrefix ? `/${environment.ftpPrefix}/v2/upload/` : '/v2/upload/');
...
export class FileUploadService {
  ...
  // uploadMultiFileOriginal(files: File[]) {
  //   const data = new FormData();
  //   files.forEach(f => {
  //     const fileName = this.utilsService.getImageName(f.name) + this.utilsService.getRandomInt(100, 5000) + '.' + f.name.split('.').pop();
