...
export class CarouselService {
  ...
  getAllBasicCarousels(searchQuery?: string) {
    let params = new HttpParams();
    if (searchQuery) {
      params = params.append('q', searchQuery);
    }
    return this.httpClient.get<{ data: Carousel[], count: number, success: boolean }>(API_CAROUSEL + 'get-all-basic', { params });
  }
  ...
  getCarouselById(id: string, select?: string) {
    ...
    return this.httpClient.get<{ data: Carousel, message: string, success: boolean }>(API_CAROUSEL +'get-by/' + id, { params });
  }
