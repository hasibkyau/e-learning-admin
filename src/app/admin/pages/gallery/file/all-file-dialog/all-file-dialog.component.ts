...
@Component({
  selector: 'app-all-file-dialog',
  templateUrl: './all-file-dialog.component.html',
  styleUrls: ['./all-file-dialog.component.scss']
})
export class AllFileDialogComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AllFileDialogComponent>,
  ...
  onSelectImage(image: Gallery) {
    ...
    const index = this.selectedFiles.findIndex(x => x._id === image._id);

    if (index === -1) {

      if (this.data.type === 'single' && this.selectedFiles.length === 1) {
        this.uiService.warn('Please select a single image');
        return;
      }

      if (this.data.type === 'multiple' && this.selectedFiles.length === this.data.count) {
  ...
  removeSelectImage(s: Gallery, event?: any) {
    ...
    const index = this.selectedFiles.findIndex(x => x._id === s._id);
    this.selectedFiles.splice(index, 1);
  }

  checkSelected(data: Gallery) {
    const index = this.selectedFiles.findIndex(x => x._id === data._id);
    return index === -1;
  }
   ...
   */
  onCloseDialog() {
    this.dialogRef.close({data: this.selectedFiles});
