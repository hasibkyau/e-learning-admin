...
@Component({
  ...
  templateUrl: './all-videos-dialog.component.html',
  styleUrls: ['./all-videos-dialog.component.scss']
})
export class AllVideosDialogComponent implements OnInit, AfterViewInit, OnDestroy {
  videoBaseLink: 'https://test.com/api/bucket/file-stream/';
  // Environment Data
  // videoBaseLink:any = environment.videoBaseLink;
  // Admin Base Data
  adminId: string;
  role: string;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AllVideosDialogComponent>,
  ...
  onSelectImage(image: Gallery) {
    ...
    if (index === -1) {
      ...
      if (this.data.type === 'single' && this.selectedImages.length === 1) {
        this.uiService.warn('Please select a single video');
        return;
      }

      if (this.data.type === 'multiple' && this.selectedImages.length === this.data.count) {
        this.uiService.warn(`You can select only ${ this.data.count} videos.`);
        return;
      }
    ...
    this.subDataFour = this.galleryService.deleteMultipleGalleryById(this.selectedIds)
      .subscribe(res => {
        ...
        if (res.success) {
          ...
          this.selectedIds = [];
          this.selectedImages = [];
          this.deleteMultipleS3Files(urls);
        } else {
          this.uiService.warn(res.message)
        }
  ...
  private deleteMultipleS3Files(data: string[]) {
    this.fileUploadService.deleteMultipleS3Files(data)
      .subscribe(res => {
