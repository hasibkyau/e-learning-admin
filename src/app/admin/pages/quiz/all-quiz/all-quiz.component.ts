...
// Admin Base Data
  ...
  ngAfterViewInit(): void {
    ...
    this.subForm = formValue
      ...
      .subscribe({
        next: (res: any) => {
          this.searchQuiz = res.data;
          this.quizs = this.searchQuiz;
      ...
      .subscribe({
        next: (res) => {
          if (res.success) {
            ...
            this.holdPrevData = this.quizs;
            this.totalQuizsStore = this.quizCount;
          }
        },
        error: (err) => {
          console.log(err);
  ...
  private deleteMultipleQuizById() {
    this.spinner.show();
    this.subDataTwo = this.quizService
      .deleteMultipleQuizById(this.selectedIds)
      .subscribe(
        (res) => {
          this.spinner.hide();
          if (res.success) {
            ...
            this.selectedIds.forEach((id) => {
              const fData = this.quizs.find((data) => data._id === id);
              if (fData) {
                selectedQuiz.push(fData);
              }
            });
  ...
  filterData(value: any, index: number, type: string) {
    switch (type) {
      case 'quiz': {
        this.filter = {...this.filter, ...{'quiz._id': value}};
