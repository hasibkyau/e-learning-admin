...
export class AppComponent implements OnInit {
  ...
  private updateMetaData() {
    // Title
    this.title.setTitle('EduPortal Admin');

    // Meta
    this.meta.addTag({name: 'author', content: 'test'});
    this.meta.addTag({name: 'copyright', content: 'test'});
    this.meta.addTag({name: 'og:locale', content: 'en_US'});
    // Open Graph
    this.meta.addTag({name: 'og:site_name', content: 'Shopify Clone'});
    // Twitter
    this.meta.addTag({name: 'twitter:card', content: 'summary_large_image'});
    this.meta.addTag({name: 'twitter:site', content: '@test'});
    this.meta.addTag({name: 'twitter:creator', content: '@test'});
  }
